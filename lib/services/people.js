...
(function() {
    ...
    var People = function(Inherits, config) {
        ...
        this.me = function(_fields, cb) {
			...
			this.createCall('GET', 'me', {projection: '(id,firstName,lastName,profilePicture(displayImage~:playableStreams))'}, cb)(this.config);
		};
        
        this.email = function(_fields, cb) {
			if (_.isFunction(_fields)) {
				cb = _fields;
				_fields = fields;
			}
			this.createCall('GET', 'emailAddress',{q: 'members', projection: '(elements*(handle~))'}, cb)(this.config);
		};
